...
const storedTokenExpiry = Number(localStorage.getItem(tokenExpiryKey) || 0);
  ...
  lessons.forEach((row, rowIndex) => {
    ...
    weekdays.forEach((_, column) => {
      ...
      if (lesson.name) {
        ...
        trigger.type = "button";
        trigger.setAttribute("aria-label", `${lesson.name}${lesson.content ? `、${lesson.content}` : ""}`);
...
function findCurrentWeekIndex(weeks) {
  ...
  weeks.forEach((week, index) => {
    ...
    const distance = Math.min(Math.abs(today.getTime() - start.getTime()), Math.abs(today.getTime() - end.getTime()));
...
function parseSheet(values, sheetMonth, sheetTitle, year) {
  ...
  return Array.from({ length: weekCount }, (_, weekIndex) => {
    ...
    const lessons = firstWeekRows.map(({ name: nameRowNumber, content: contentRowNumber }, periodIndex) => {
      ...
      return weekdays.map((_, dayIndex) => {
        ...
        if (shouldHideLesson(dates[dayIndex], periodIndex)) return { name: "", content: "" };
        const name = String(nameRow[columnIndex] || "").trim();
        return { name, content: name ? String(contentRow[columnIndex] || "").trim() : "" };
      });
    });
...
async function fetchMonthSheetTitles() {
  ...
  if (!data) return null;
  return data.sheets.map((sheet) => sheet.properties?.title || "").map((title) => {
...
async function fetchAllMonthValues(monthSheets) {
  ...
  monthSheets.forEach(({ title }) => endpoint.searchParams.append("ranges", `'${title}'!${config.cellRange}`));
  const data = await googleApiFetch(endpoint);
...
function mergeMonthWeeks(monthSheets, valueRanges) {
  ...
  monthSheets.forEach((sheet, index) => {
    parseSheet(valueRanges[index]?.values || [], sheet.month, sheet.title, year).forEach((week) => {
      const key = week.dates.filter(Boolean).join(",");
      if (key && !seen.has(key)) { seen.add(key); weeks.push(week); }
    });
  });
...
async function fetchTimetable({ manual = false } = {}) {
  ...
  if (!accessToken) {
    requestAccessToken({ interactive: !hasPreviousAuthorization(), manual });
    return;
  }
  setLoading(true);
  showNotice("");
  try {
    ...
    if (!monthSheets.length) throw new Error("「○月」という名前の時間割シートが見つかりません。");
    ...
    if (!weeks.length) throw new Error("月別シートに時間割データがありません。");
...
function requestAccessToken({ interactive = true, manual = false } = {}) {
  ...
  if (!window.google?.accounts?.oauth2) {
    if (interactive) showNotice("Googleログインの準備中です。数秒後にもう一度お試しください。");
    return;
  }
  ...
  tokenClient ||= window.google.accounts.oauth2.initTokenClient({
    ...
    callback: async (response) => {
      ...
      if (response.error) {
        ...
        if (request.interactive) showNotice("Googleログインを完了できませんでした。");
        return;
      }
      accessToken = response.access_token;
      localStorage.setItem("googleAccessToken", accessToken);
      localStorage.setItem(tokenExpiryKey, String(Date.now() + Math.max(0, Number(response.expires_in) - 60) * 1000));
...
function initialize() {
  ...
  elements.signInButton.hidden = Boolean(accessToken) || hasPreviousAuthorization() || !configured;
  if (!configured) {
    showNotice("現在はサンプル表示です。config.js にGoogle OAuthとスプレッドシートを設定すると実データへ切り替わります。");
  } else if (accessToken) {
    fetchProfile();
...
elements.signInButton.addEventListener("click", () => requestAccessToken({ interactive: true }));
elements.accountButton.addEventListener("click", () => requestAccessToken({ interactive: true }));
